        ...
        if (args.size() != argAmount) {
            ...
            for (int i = 0; i < collection.size(); i++) {
                ...
                if (idToFind == collection.get(i).getId()) {
                    ...
                    curMovie.setName(askName());

                    curMovie.setCoordinates(new Coordinates(askX(), askY()));

                    curMovie.setCreationDate(LocalDateTime.now());

                    curMovie.setOscarsCount(askOscarsCount());

                    curMovie.setTotalBoxOffice(askTotalBoxOffice());

                    curMovie.setUsaBoxOffice(askUsaBoxOffice());

                    curMovie.setGenre(askGenre());

                    curMovie.setOperator(new Person(askOperatorName(), askBirthdate(), askWeight(), askEyeColor(), askNationality()));
                    return;
                }
            }
        }
